%% This is the runme file for NED, the ME 410 final exam
% Originally created 18 November, 2022, for ME 410
...
%% Compile the Matlab mex function
mex -g mex_Exam2_function.cpp HolopticSensorFusion.cpp AutoControl.cpp
...
q = [0.9763;0;0.2164;0]; %[e0;e1;e2;e3] Initial estimate of quaternion orientation
VI = [0;0;0]; %[VxI;VyI;VzI] (m/s) initial GPS inertial velocity
XI = [0;0;0]; %[xI;yX;zI] (m) initial GPS inertial displacement
GPS_lat = 43.80960;...[lat] (deg) latitude angle
GPS_lon = -111.78573;...[lon] (deg) longitude angle
GPS_alt = 0;...[-zI] (m) altitude above earth
lat0 = GPS_lat; ...[lat] (deg) initial latitude
lon0 = GPS_lon; ...[lon] (deg) initial longitude
alt0 = 20; ...(m) initial altitude
waypointIndex = 0; ...(-) index of current waypoint
...
c.CTz_beta = 0.002; %[-] Sideslip yaw torque coefficient
...
wind.ThereIsWind = 0; %[0 or 1] flag to turn wind on (1) or off (0)
...
while x(9) < 0, ii = ii + 1;
    ...
    %get the RI2b rotation matrix
    RI2b = R_I2b([x(10:end)]);

    %get the sensor readings
    gyro = [x(4);x(5);x(6)]; %(rad/s) gyrometer sensor signals
    accel = RI2b*[0;0;9.806]-dV; %(m/s2) accelerometer sensor signal
    magnet = 53*RI2b*[cosd(67);0;sind(67)];%(uT) magnetometer sensor signal
    newGPS = 0; %(0 or 1) Indicate if new GPS information is available
    if (t(ii) - tGPS) >= 0.1
        dtGPS = t(ii) - tGPS; %(s) GPS time-step
        tGPS = t(ii);
        [GPS_lat,GPS_lon] = getLatitudeLongitude(x(7), x(8), lat0, lon0);
        GPS_alt = -x(9);
        newGPS = 1; %(0 or 1) new GPS info is available
    end

    %Get the commands
    [delta_t,...[0 to 1] throttle command
    ...
    %If all of the waypoints have been reached, stop the simulation
    if wpIndex > height(GPSwaypoints)-1
        break;
    end
